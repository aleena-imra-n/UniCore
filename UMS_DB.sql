...
CREATE TABLE COURSES (
    ...
    recommended_semester TINYINT        NOT NULL DEFAULT 1,  -- semester this course belongs to (1-8)
...
--     Now enforces:
--       a) Semester gating: course.recommended_semester <= student.current_semester
--       b) Prerequisite pre-filter: mandatory prereqs must be completed with required grade
--       c) Excludes courses the student has already completed (those go to improvement view)
...
WHERE sem.is_active = 1
  -- (a) Semester gating: only show courses for current or earlier semesters
  AND c.recommended_semester <= s.current_semester
  -- (b) Prerequisite pre-filter: exclude courses with unmet mandatory prereqs
  AND NOT EXISTS (
      SELECT 1
  ...
  -- (c) Exclude courses already completed (those go to improvement view)
  AND NOT EXISTS (
      SELECT 1
      FROM   ENROLLMENTS      e3
...
-- V2b: Courses available for improvement (already passed, offered again this semester)
...
WHERE sem.is_active = 1
  -- Student must have completed this course at least once
  AND EXISTS (
      SELECT 1
      FROM   ENROLLMENTS      e4
  ...
  -- Not currently enrolled in this offering
  AND NOT EXISTS (
      SELECT 1
      FROM   ENROLLMENTS e5
...
INSERT INTO COURSES
    (course_id, major_dept_id, course_code, course_name, credit_hours, category_id, recommended_semester)
VALUES
    (1,  1, N'CS101', N'Programming Fundamentals',        3, 1, 1),  -- semester 1
    (2,  1, N'CS201', N'Data Structures & Algorithms',    3, 1, 3),  -- semester 3
    (3,  1, N'CS301', N'Database Systems',                3, 1, 5),  -- semester 5
    (4,  1, N'AI201', N'Machine Learning',                3, 2, 3),  -- semester 3
    (5,  1, N'DS201', N'Data Analytics & Visualization',  3, 2, 3),  -- semester 3
    (6,  1, N'CY201', N'Network Security',                3, 2, 3),  -- semester 3
    (7,  1, N'SE201', N'Software Design Patterns',        3, 2, 3),  -- semester 3
    (8,  2, N'CE101', N'Digital Logic Design',            3, 1, 1),  -- semester 1
    (9,  2, N'EE101', N'Circuit Analysis',                3, 1, 1),  -- semester 1
    (10, 2, N'CE201', N'Microprocessor Systems',          3, 2, 3),  -- semester 3
    (11, 3, N'BA101', N'Principles of Management',        3, 1, 1),  -- semester 1
    (12, 3, N'AF201', N'Financial Accounting',            3, 1, 3),  -- semester 3
    (13, 3, N'FT201', N'FinTech Fundamentals',            3, 2, 3);  -- semester 3
