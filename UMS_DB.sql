...
CREATE TABLE COURSES (
    ...
    major_dept_id   INT            NOT NULL,
    category_id     INT            NOT NULL,   -- ✅ included directly
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE cp.course_id = @course_id
          AND (
                -- ❗ If you DON'T have is_mandatory column, remove this line
                cp.is_mandatory = 1
              )
          AND NOT EXISTS (
              SELECT 1
    ...
    -- ────────────────────────────────────────────────
    -- 6. CREDIT LIMIT CHECK (YOUR ADDED LOGIC)
    -- ────────────────────────────────────────────────
    ...
    BEGIN
        PRINT 'WARNING: Exceeding standard credit limit (overload).';
    END
...
-- Add this block inside SP_RegisterStudent, after the prerequisite check
-- and before the capacity check
...
-- Get the semester and credits of the course being registered
SELECT @semester_id       = co.semester_id,
       @new_course_credits = c.credit_hours
FROM   COURSE_OFFERINGS co
JOIN   COURSES c ON c.course_id = co.course_id
WHERE  co.offering_id = @offering_id;

-- Get student's sub-dept
SELECT @sub_dept_id = sub_dept_id
FROM   STUDENTS WHERE student_id = @student_id;

-- Get policy limits for this sub-dept
SELECT @max_credits      = max_credits,
       @overload_credits = overload_credits
FROM   CREDIT_POLICIES
WHERE  sub_dept_id = @sub_dept_id AND is_active = 1;

-- Get current enrolled credits this semester
SELECT @current_credits = ISNULL(SUM(c2.credit_hours), 0)
FROM   ENROLLMENTS e2
...
-- Block if adding this course exceeds max (hard block at overload limit)
IF (@current_credits + @new_course_credits) > @overload_credits
BEGIN
    RAISERROR('Credit hour limit exceeded. Maximum allowed: %d. Currently enrolled: %d. Course credits: %d.',
              16, 1, @overload_credits, @current_credits, @new_course_credits);
    RETURN;
END

-- Warn if exceeding standard max but within overload limit
-- (soft warning — still allows registration, frontend shows a caution)
IF (@current_credits + @new_course_credits) > @max_credits
BEGIN
    PRINT 'WARNING: Enrolling above standard credit limit. Dean approval may be required.';
END
-- ================================================================
-- PART 4: DUMMY DATA
-- ================================================================
...
-- 4.7 COURSES
-- ────────────────────────────────────────────────────────────────
-- COURSE_CATEGORIES must be inserted BEFORE COURSES (FK dependency)
...
INSERT INTO CREDIT_POLICIES (sub_dept_id, min_credits, max_credits, overload_credits) VALUES
    ...
    (6,  12, 21, 24),   -- CE
    (7,  12, 21, 24),   -- EE
    (8,  12, 18, 21),   -- AF  (management programs typically lower max)
    (9,  12, 18, 21),   -- BA
    (10, 12, 18, 21);   -- FINTECH
GO
...
-- 2. Show a few sample rows so we can see what values are actually stored
